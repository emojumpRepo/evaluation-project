...
public class AssessmentResultPdfGenerator {
    ...
    /**
     * 加载可显示中文的字体：优先加载类路径中的字体文件；
     * 失败时尝试系统字体；仍失败则使用UTF-8编码的Helvetica并记录警告。
     */
    private static PDFont loadCjkFont(PDDocument document) {
        ...
        for (String resourcePath : resourceFonts) {
            try {
                java.io.InputStream is = AssessmentResultPdfGenerator.class.getResourceAsStream(resourcePath);
                if (is != null) {
        ...
        // 3. 最后的备选方案：使用Helvetica但记录警告
        System.err.println("警告: 无法加载中文字体，使用Helvetica字体。中文字符可能显示为方块。");
        System.err.println("建议: 将中文字体文件（如NotoSansSC-Regular.ttf）放置在src/main/resources/fonts/目录下");
